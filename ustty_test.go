...
func TestUSTTYEncodeChar(t *testing.T) {
	tt := []struct {
		caseName    string
		char        rune
		charset     Charset
...
func TestUSTTYDecodeChar(t *testing.T) {
	tt := []struct {
		caseName    string
		code        byte
		charset     Charset
...
func TestUSTTYEncode(t *testing.T) {
	tt := []struct {
		caseName   string
		msg        string
		ignErr     bool
		...
		},
		{
			...
			msg:        "1 + 1 = 2",
			ignErr:     true,
			expect:     []byte{0, 31, 27, 23, 4, 4, 23, 4, 4, 19},
			shouldFail: true,
			failedText: fmt.Sprintf("expect %v, got %%v", []byte{0, 31, 27, 23, 4, 4, 23, 4, 4, 19}),
		},
	}
...
func TestUSTTYDecode(t *testing.T) {
	tt := []struct {
		caseName   string
		codes      []byte
		ignErr     bool
